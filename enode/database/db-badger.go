...
func (bd *badgerDb) Has(k []byte) bool {
	...
	bd.db.View(func(txn *badger.Txn) error { //db.View相当于打开了一个读写事务:db.NewTransaction(true)。用db.Update的好处在于不用显式调用Txn.Discard()了
		_, err := txn.Get(k)
		if err != nil {
			return err
		} else {
...
func (bd *badgerDb) IterDB(fn func(k, v []byte) error) int64 {
	...
	var total int64
	bd.db.View(func(txn *badger.Txn) error {
